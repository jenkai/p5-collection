...
function setup() {
	...
	for (let i = 0; i < n * 2; i++) {
		...
		let r_len = map(noise((Math.cos(base_angle) + 1) /2, (Math.sin(base_angle) + 1) /2), 0, 1, -300, 200)
		r2.push(r_len)
	...
	for (let i = 0; i < n; i++) {
		...
		let p3 = new p5.Vector(Math.cos(angle) * (r + r2[i + n]), Math.sin(angle) * (r + r2[i + n]))
		
		strokeWeight(0.5)
		let hue = map(noise(Math.cos(angle) + 1, Math.sin(angle) + 1), 0, 1, 30, 100)
